...
def get_gateway_url(project_id: str) -> str:
    """Discover API Gateway URL via gcloud."""
    try:
        result = subprocess.run(
            ["gcloud", "api-gateway", "gateways", "describe", "consult-mcp-gw",
             f"--project={project_id}", "--location=us-central1",
...
def get_api_key(project_id: str) -> str:
    """Retrieve the string value for the managed API key."""
    try:
        # The resource name in TF is 'consult-mcp-key'
        result = subprocess.run(
            ["gcloud", "services", "api-keys", "get-key-string", "consult-mcp-key",
...
def cmd_status(args, provider, project_id):
    ...
    if url:
        print(f"URL:     {url}")
        key = get_api_key(project_id)
        if key:
            print(f"API Key: {key[:8]}...")
        else:
            print(f"API Key: MISSING")
    else:
        # Fallback to Cloud Run check (though it's private now)
        res = subprocess.run(["gcloud", "run", "services", "describe", "consult-mcp", f"--project={project_id}", "--region=us-central1", "--format=value(status.url)"], capture_output=True, text=True)
        cr_url = res.stdout.strip()
        if cr_url:
            print(f"URL:     {cr_url} (Direct Cloud Run - Private)")
        else:
...
def cmd_export(args, provider, project_id):
    """Output URL + tokens as YAML for user import.
    ...
    if url:
        ...
        error("API Gateway not found. Run: ./cloud deploy")
        sys.exit(1)
