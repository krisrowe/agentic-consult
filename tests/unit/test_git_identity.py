...
from agentic_consult.scanner import check_git_identity
...
def test_identity_pristine_repo_pass(tmp_path):
    ...
    with patch.dict(os.environ, {'GIT_AUTHOR_EMAIL': 'user@example.com'}):
        issues = check_git_identity(repo)
    assert not issues
...
def test_identity_pristine_history_but_impending_mismatch_fail(tmp_path):
    ...
    assert issues
    assert "Identity Mismatch" in issues[0]
    assert "inconsistent with impending identity" in issues[0]
...
def test_identity_explicit_local_config_unpushed_fail(tmp_path):
    ...
    assert issues
    assert "Unpushed work is inconsistent" in issues[0]
...
def test_identity_impending_mismatch_local_fail(tmp_path):
    ...
    assert issues
    assert "Impending commit identity 'test@fake.com' differs from configured local identity" in issues[0]
