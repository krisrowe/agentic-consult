...
def test_schema_disallows_additional_properties_globally(mock_config_dir):
    """
    ...
    def build_valid_minimal(s):
        ...
        if s.get("type") == "object":
            ...
            for prop, sub in s.get("properties", {}).items():
                ...
                if sub.get("type") == "string": obj[prop] = "val"
                elif sub.get("type") == "boolean": obj[prop] = True
                elif sub.get("type") == "integer": obj[prop] = 1
                elif sub.get("type") == "array": obj[prop] = []
                elif sub.get("type") == "object": obj[prop] = build_valid_minimal(sub)
            return obj
        return "val"
    ...
    for path in all_paths:
        if "_items_" in path: continue # Skipping array items for now to keep it simple
